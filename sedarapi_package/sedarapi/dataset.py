...
class Dataset:
    #--------------------------------------------------------------
    def __init__(self, connection: Type[Commons], workspace_id: str, dataset_id: str):
        ...
        self.isFavorite = self.content["isFavorite"]
    ...
    def update_datasource(self, datasource_definition, file_paths) -> Dataset:
        """
        ...
        Args:
            ...
            file_paths: The path to the datasource file or a dictionary containing 
                                    multiple file paths with keys being the datasource names and 
                                    values being their respective paths.
        ...
        Description:
            ...
            The dataset's datasource can be updated using the provided definition and file paths.
        ...
        Example:
            ...
            ```
        """
        self._update_datasource(self.workspace, self.id, datasource_definition, file_paths)
    ...
    def ingest(self) -> dict:
        """
        Initiates the ingestion process for the current dataset.
    ...
    def start_profiling(self, dataset_version="CURRENT REVISION") -> bool:
        """
    ...
    ##############################
    # Dataset Notebook Interface #
    ##############################
    def get_notebooks(self) -> List[Notebook]:
        """
        Retrieves all notebooks associated with the dataset.
    ...
    def add_notebook(self, title: str, description: str, type: str = "JUPYTER", is_public: bool = True, version: str = "LATEST") -> Notebook:
        ...
        ```
    """
        return Notebook(self.connection, self.workspace, self.id, self._add_notebook(self.workspace, self.id, title, description, type, is_public, version)["id"])
    
    def get_notebook_code(self, notebook_type: str = "JUPYTER") -> str:
        """
        Retrieves the notebook code for the dataset.

        Args:
            notebook_type (str, optional): The type of the notebook (e.g., JUPYTER). Defaults to "JUPYTER".

        Returns:
            str: The code for the specified notebook type.

        Raises:
            Exception: If there's an error during the notebook code retrieval process.

        Description:
            This method retrieves the code for the specified notebook type by sending a GET request to the '/api/v1/workspaces/{workspace_id}/notebook-code/{dataset_id}' endpoint.

        Example:
        ```python
        dataset = workspace.get_all_datasets()[0]
        try:
            notebook_code = dataset.get_notebook_code(notebook_type="JUPYTER")
            print(notebook_code)
        except Exception as e:
            print(e)
        ```
    """
        return self._get_notebook_code(self.workspace,self.id, notebook_type)["code"]
    

    ##############################
    # Dataset Cleaning Interface #
    ##############################
    def get_cleaner(self, dataset_version="CURRENT REVISION"):
        """
    ...
    def get_all_attributes(self):
        """
        Retrieves all attributes associated with the current dataset.

        Returns:
            list[Attribute]: A list of Attribute objects representing all attributes associated with the dataset.
    ...
    def add_user_permission(self, user: Type[User], can_read: bool = True, can_write: bool = True, can_delete: bool = True) -> dict:
        """
    ...
    def edit_user_permission(self, user: Type[User], can_read = None, can_write = None, can_delete = None) -> dict:
        """
    ...
    def remove_user_permission(self, user : Type[User]) -> dict:
        """
    ...
    def create_dataset_link(self, linked_dataset: Type[Dataset], description: str) -> dict:
        """
    ...
    def delete_dataset_link(self, linked_dataset: Type[Dataset]) -> bool:
        """
    ...
    def update_dataset_link(self, linked_dataset: Type[Dataset], description: str = None):
        """
    ...
    def get_linked_dataset_info(self, linked_dataset: Type[Dataset]):
        """
    ...
    def compare_deltas(self, revision_a: str, revision_b: str = "CURRENT REVISION"):
        """
        Compares the dataset deltas between two revisions.

        Args:
            revision_a (str): The identifier for the first revision to compare.
            revision_b (str, optional): The identifier for the second revision to compare. Defaults to "CURRENT REVISION", which represents the current revision of the dataset.
        ...
        Description:
            This method compares the dataset deltas between two specified revisions by sending a GET request to the '/api/v1/workspaces/{workspace_id}/datasets/{dataset_id}/deltas' endpoint. If the user does not specify the second revision, the method uses the current revision of the dataset as the default.
            
            It's important to note that this method might take a significant amount of time (several minutes) to complete. The response is only provided once the comparison is finished. 

        Example:
        ```python
        dataset = workspace.get_all_datasets()[0]
        try:
            comparison_result = dataset.compare_deltas("revision_1", "revision_2")
            print(comparison_result)
        except Exception as e:
            print(e)
        ```
        """
        # If the user does not specify a revision, the currentRevision is taken as the second parameter.
        if revision_b == "CURRENT REVISION":
            revision_b = self.content["datasource"]["currentRevision"]
    ...
    #--------------------------------------------------------------
    def _update_dataset(self, workspace_id, dataset_id, title, description, author, longitude, 
    ...
    #--------------------------------------------------------------
    def _update_datasource(self, workspace_id, dataset_id, datasource_definition, file_paths):
        ...
        if isinstance(file_paths, str):
            ...
            if os.path.exists(file_paths):
                files = {self.connection._remove_file_extension(file_name): (file_name, open(file_paths, 'rb'), "application/vnd.ms-excel")}
            else:
        ...
        # If we got a dictionary with multiple file_paths
            ...
            for key, path in file_paths.items():
                ...
                if os.path.exists(path):
                    files[self.connection._remove_file_extension(file_name)] = (file_name, open(path, 'rb'), "application/vnd.ms-excel")
                else:
                    self.logger.error(f"Datasource File not found: {path}")
                    return None
    ...
    #--------------------------------------------------------------
    def _get_notebook_code(self, workspace_id, dataset_id, notebook_type):
        resource_path = f"/api/v1/workspaces/{workspace_id}/notebook-code/{dataset_id}"
        payload = {
        ...
        response = self.connection._get_resource(resource_path, payload)
        if response is None:
            raise Exception("The Notebook Code could not be retrieved. Set the logger level to \"Error\" or below to get more detailed information.")

        self.logger.info(f"The Notebook Code for Dataset '{dataset_id}' has been retrieved successfully.")
        return response
